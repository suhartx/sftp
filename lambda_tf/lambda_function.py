...
def lambda_handler(event, context):
    ...
    source_dir2 = "sftps3mixer/{}/escritura/{}".format(persona,nombreFichero)#ORIGEN CON CABECERA
    target_dir = "Peru/lectura/{}".format(nombreFichero)#DESTINO

    # let's say you have a new file in Landing folder, the s3 uri is
    s3_uri = "s3://sftps3mixer/"+file_key#ORIGEN CON CABECERA
    ...
    #COMPROBANTE SI ES AJUSTADOREA IKUSKATZAILEA O KLIENTEA SI LO ES HACE UN CAMBIO DE VARIABLES
    
    if persona.__eq__("Ajustadoreak") or persona.__eq__("Ikuskatzaileak")or persona.__eq__("Klienteak"):
        ...
        target_dir = "Peru/lectura/{}".format(nombreFichero)
        source_dir2="sftps3mixer/{}".format(source_dir)

    #COMPROBANTE SI ES MIXER BILBO O DONOSTI SI LO ES HACE UN CAMBIO DE VARIABLES

    if persona.__eq__("MixerDonosti"):
        target_dir = "MixerBilbo/lectura/{}".format(nombreFichero)
    if persona.__eq__("MixerBilbo"):
        target_dir = "MixerDonosti/lectura/{}".format(nombreFichero)
    ...
    html_body = ("<html>"
        ...
        "<p>condiciones():{}</p>"
        "</body>"
        "</html>").format(persona,nombreFichero,fecha,tamanyo,s3_uri,source_dir, target_dir, tipo, nombreFichero, (nombreFichero.__eq__("")) , persona.__eq__("Peru") or tipo.__eq__("lectura")or (tipo.__eq__("escritura") and (nombreFichero.__eq__("")) ))
    ...
    try:
        ...
        if persona.__eq__("Peru") or tipo.__eq__("lectura")or (tipo.__eq__("escritura")and (nombreFichero.__eq__(""))  or tamanyoSinT==0):
            print("mal")
        else:
            #PREOCEDIMIENTO DE REUBICACIÓN DEL FICHERO
            s3_resource = boto3.resource('s3')
            print("funtziona")
            s3_resource.Object("sftps3mixer", target_dir).copy_from(
            CopySource=source_dir2)
            print("hasta aqui")
            # Delete the former object A
            s3_resource.Object("sftps3mixer", source_dir).delete()   
